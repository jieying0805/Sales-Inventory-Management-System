	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && customerName && orderStatus) {
	...
	$.ajax({
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;"">'+
					'<input type="text" name="commission[]" id="commission'+count+'" autocomplete="off" disabled="true" class="form-control" />'+
					'<input type="hidden" name="commissionValue[]" id="commissionValue'+count+'" autocomplete="off" class="form-control" />'+
				'</td style="padding-left:20px;">'+				
				'<td style="padding-left:10px;">'+
					'<div class="form-group">'+
				...
				'</td>'+
				'<td style="padding-left:20px;">'+
					'<input type="text" name="totalComm[]" id="totalComm'+count+'" autocomplete="off" class="form-control" disabled="true" />'+
					'<input type="hidden" name="totalCommValue[]" id="totalCommValue'+count+'" autocomplete="off" class="form-control" />'+
				'</td>'+
				'<td>'+
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#commission"+row).val(response.commission);
					$("#commissionValue"+row).val(response.commission);
					...
					var totalComm = Number(response.commission) * 1;
					totalComm = totalComm.toFixed(2);
					$("#totalComm"+row).val(totalComm);
					$("#totalCommValue"+row).val(totalComm);	
					
					subAmount();
					calculateShipping(totalQty);
...
function getTotal(row = null) {
    if (row) {
        ...
        var totalComm = Number($("#commission" + row).val()) * Number($("#quantity" + row).val());
        totalComm = totalComm.toFixed(2);
        $("#totalComm" + row).val(totalComm);
        $("#totalCommValue" + row).val(totalComm);
        ...
        // Loop through all rows to calculate the accumulated quantity
        for (var i = 1; i <= row; i++) {
            totalQty += Number($("#quantity" + i).val());
        }

        var shipping = calculateShipping(totalQty);

        $("#shipping").val(shipping);
        $("#shippingValue").val(shipping);
...
function subAmount() {
    ...
    for (var x = 0; x < tableProductLength; x++) {
        ...
        totalCommAmount = Number(totalCommAmount) + Number($("#totalComm" + count).val());
    } // /for

    totalSubAmount = totalSubAmount.toFixed(2);
    totalCommAmount = totalCommAmount.toFixed(2);
    ...
    // sub totalComm
    $("#subTotalComm").val(totalCommAmount);
    $("#subTotalCommValue").val(totalCommAmount);

    // total amount
    var totalAmount = (Number($("#subTotal").val()) + Number($("#shipping").val()));
		...
		if(customerId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#customerPhone").val(response.cust_phone);
					$("#customerAddress").val(response.cust_address);
