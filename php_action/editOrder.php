...
if($_POST) {	
  ...
  $userid = $_SESSION['userId'];
	...
	$sql = "UPDATE orders SET order_date = '$orderDate', cust_id = '$cust_id', sub_total = '$subTotalValue', shipping = '$shipping', total_amount = '$totalAmountValue', total_commission = '$subTotalCommValue', payment_type = '$paymentType', payment_ref = '$paymentRef', order_status = '$orderStatus', tracking = '$tracking',user_id = '$userid' WHERE order_id = {$orderId}";	
	$connect->query($sql);
		...
		for($x = 0; $x < count($_POST['productName']); $x++) {			
			...
			while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
				$updateQuantity[$x] = $updateProductQuantityResult[0] - $_POST['quantity'][$x];							
					...
					// add into order_item
				$orderItemSql = "INSERT INTO order_item (order_id, product_id, quantity, price, commission, total, total_commission) 
				VALUES ({$orderId}, '".$_POST['productName'][$x]."', '".$_POST['quantity'][$x]."', '".$_POST['priceValue'][$x]."', '".$_POST['commissionValue'][$x]."', '".$_POST['totalValue'][$x]."', '".$_POST['totalCommValue'][$x]."')";
